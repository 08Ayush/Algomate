...
export async function POST(
  ...
  try {
    ...
    // Only publishers can unpublish timetables
    if (user.faculty_type !== 'publisher') {
      return NextResponse.json(
        { success: false, error: 'Only publishers can unpublish timetables.' },
        { status: 403 }
      );
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { success: false, error: `Failed to unpublish: ${updateError.message}` },
        { status: 500 }
      );
    }

    // Add workflow unpublish record
    ...
    console.error('Unexpected error unpublishing timetable:', error);
    return NextResponse.json(
