...
export const dbHelpers = {
    ...
    async getDepartments(): Promise<Department[]> {
        try {
            ...
            // Note: This fetch logic in backend server-side code might need absolute URL, but assuming this helper runs in API routes
            const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/departments`)
            if (!response.ok) {
